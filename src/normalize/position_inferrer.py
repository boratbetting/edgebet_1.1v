...
def infer_position(avg_att, avg_blk, avg_rec, avg_pts, matches, libero_flag, league="FR_LAF_W"):
    if libero_flag == "L": return "L", "HIGH", "libero_flag"
    if league == "FR_LAM":
        if avg_att == 0 and avg_blk == 0 and avg_rec > 5: return "L","MEDIUM","att=0 blk=0 rec>5"
        if avg_rec >= 15 and avg_att <= 5 and avg_blk <= 1.5: return "L","MEDIUM",f"rec={avg_rec:.1f}"
        if avg_att >= 12.0 and avg_rec <= 5.0: return "OPP","HIGH",f"att={avg_att:.1f}"
        if avg_att >= 8.0 and avg_rec <= 3.0 and avg_att > avg_blk*4: return "OPP","HIGH","att/blk ratio"
        if avg_att >= 6.0 and avg_rec <= 5.0 and avg_att > avg_blk*3: return "OPP","MEDIUM",f"att={avg_att:.1f}"
        if avg_blk >= 2.0 and avg_rec <= 5.0 and avg_blk > avg_att*0.3: return "MB","HIGH",f"blk={avg_blk:.1f}"
        if avg_blk >= 1.2 and avg_rec <= 3.0: return "MB","MEDIUM",f"blk={avg_blk:.1f}"
        if avg_att >= 5.0 and avg_rec >= 5.0: return "OH","HIGH",f"att={avg_att:.1f} rec={avg_rec:.1f}"
        if avg_att >= 3.0 and avg_rec >= 3.0: return "OH","MEDIUM",f"att={avg_att:.1f} rec={avg_rec:.1f}"
        if avg_blk < 0.5 and avg_att < 2.0 and avg_rec < 2.0 and matches >= 8: return "S","MEDIUM","low_all"
        if avg_pts <= 3.0 and avg_blk < 0.5 and avg_att < 2.0 and matches >= 5: return "S","LOW","low_stats"
        if avg_blk >= 0.5 and avg_att < 7.0 and avg_rec <= 5.0: return "MB","LOW",f"blk={avg_blk:.1f}"
...
def build_position_table(clean_players, league="FR_LAF_W"):
    ...
    for name,a in agg.items():
        ...
        pos,conf,reason=infer_position(
            ...
            round(a["rec"]/n,2),round(a["pts"]/n,2),a["n"],a["lib"],league)
